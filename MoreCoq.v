...
Proof.
  ...
  Case "n = S n'".
    ...
    SCase "m = S m'".
      ...
      inversion eq. apply IHn' in H0. rewrite H0. reflexivity.  Qed.
(** [] *)
...
Proof.
  ...
  Case "n = S n'".
    intros m eq. destruct m as [| m'].
    SCase "m = 0". inversion eq.
    SCase "m = S m'". inversion eq. f_equal. apply IHn'. assumption.  Qed.
(** [] *)
...
Theorem index_after_last: forall (n : nat) (X : Type) (l : list X),
     ...
     index n l = None.
Proof.
  intros n X l. generalize dependent n. induction l as [| v' l'].
  Case "l = []".
    intros n eq. rewrite <- eq. reflexivity.
  Case "l = v' :: l'".
    ...
    SCase "n = 0". inversion eq.
    SCase "n = S n'".
      inversion eq. apply IHl'. reflexivity.  Qed.
(** [] *)
...
Proof.
  ...
  Case "l1 = v1' :: l1'".
    intros n eq. destruct n as [| n'].
    SCase "n = 0". inversion eq.
    SCase "n = S n'". f_equal. apply IHl1'. inversion eq. reflexivity.  Qed.
(** [] *)

(** **** Exercise: 4 stars, optional (app_length_twice)  *)
(** Prove this by induction on [l], without using app_length. *)

Lemma length_add_middle : forall (X : Type) (l1 l2 : list X) (v : X),
  length (l1 ++ v :: l2) = S (length (l1 ++ l2)).
Proof.
  intros X l1 l2 v. induction l1 as [| v1' l1'].
  Case "l1 = []". reflexivity.
  Case "l1 = v1' :: l1'". simpl. rewrite IHl1'. reflexivity.  Qed.

Theorem app_length_twice : forall (X:Type) (n:nat) (l:list X),
     length l = n ->
     length (l ++ l) = n + n.
Proof.
  intros X n l. generalize dependent n. induction l as [| v' l'].
  Case "l = []".
  ...
  Case "l = v' :: l'".
    ...
    SCase "n = 0". inversion eq.
    SCase "n = S n'". simpl.
    rewrite length_add_middle. rewrite <- plus_n_Sm.
    f_equal. f_equal. apply IHl'. inversion eq. reflexivity.  Qed.
(** [] *)
...
Theorem double_induction: forall (P : nat -> nat -> Prop),
  ...
  forall m n, P m n.
Proof.
  intros P Pz Pmz Pzn Pmn. induction m as [| m'].
  Case "m = 0".
    induction n as [| n'].
    SCase "n = 0". apply Pz.
    SCase "n = S n'". apply Pzn. apply IHn'.
  Case "m = S m'".
    induction n as [| n'].
    SCase "n = 0". apply Pmz. apply IHm'.
    SCase "n = S n'". apply Pmn. apply IHm'. Qed.
(** [] *)
...
Proof.
  unfold override.
  intros X x1 x2 k1 k2 f. destruct (beq_nat k1 k2).
...
Proof.
  ...
  Case "l = (x', y') :: l'".
    ...
    destruct l1 as [| v1' l1'].
    SCase "l1 = []". inversion eq.
    SCase "l1 = v1' :: l1'".
      destruct l2 as [| v2' l2'].
      SSCase "l2 = []". inversion eq.
      SSCase "l2 = v2' :: l2'".
        inversion eq. simpl. f_equal. apply IHl'.
        f_equal. assumption. assumption.  Qed.
(** [] *)
...
Proof.
  intros f b.
  destruct (f true) eqn: eq1; destruct (f false) eqn: eq2; destruct b;
    repeat (rewrite eq1 || rewrite eq2); reflexivity.  Qed.
(** [] *)
...
Proof.
  unfold override.
  intros X x1 k1 k2 f eq. destruct (beq_nat k1 k2) eqn: eqb.
  Case "beq_nat k1 k2 = true".
    rewrite <- eq. f_equal. apply beq_nat_true. apply eqb.
...
Theorem beq_nat_trans : forall n m p,
  ...
  beq_nat n p = true.
Proof.
  induction n as [| n']; destruct m as [| m']; destruct p as [| p'];
    simpl; intros; eauto; congruence.  Qed.
(** [] *)
...
Proof.
  ...
  Case "l1 = v1' :: l1'".
    ...
    SCase "l2 = []".
      inversion eq1.
    SCase "l2 = v2' :: l2'".
      destruct l as [| [l'1 l'2] l'].
      SSCase "l = []". inversion eq2.
      SSCase "l = (l'1, l'2) :: l'".
        simpl in *.
        inversion eq1. inversion eq2.
        rewrite H3. rewrite IHl1' with (l2:=l2').
        reflexivity. assumption. assumption.  Qed.
(** [] *)
...
Proof.
  ...
  Case "beq_nat k1 k3 = true".
    ...
    SCase "beq_nat k2 k3 = true".
      rewrite beq_nat_sym in eq1.
      rewrite beq_nat_trans with (m:=k3) in eq.
...
Proof.
  ...
  Case "l = v' :: l'".
    simpl. intros lf eq. destruct (test v') eqn: eqt.
    SCase "test v' = true".
      inversion eq. subst. apply eqt.
    SCase "test v' = false".
      apply IHl' with (lf:=lf). apply eq.  Qed.
(** [] *)
...
Fixpoint forallb {X : Type} (f : X -> bool) (ls : list X) : bool :=
  match ls with
  | [] => true
  | h :: ls' => andb (f h) (forallb f ls')
  end.

Fixpoint existsb {X : Type} (f : X -> bool) (ls : list X) : bool :=
  match ls with
  | [] => false
  | h :: ls' => orb (f h) (existsb f ls')
  end.

Example test_forallb1_1 : forallb oddb [1;3;5;7;9] = true.
Proof. reflexivity.  Qed.
Example test_forallb1_2 : forallb negb [false;false] = true.
Proof. reflexivity.  Qed.
Example test_forallb1_3 : forallb evenb [0;2;4;5] = false.
Proof. reflexivity.  Qed.
Example test_forallb1_4 : forallb (beq_nat 5) [] = true.
Proof. reflexivity.  Qed.
...
Proof. reflexivity.  Qed.
Example test_existsb_3 : existsb evenb [] = false.
Proof. reflexivity.  Qed.

Definition existsb' {X : Type} (f : X -> bool) (ls : list X) :=
  negb (forallb (fun x => negb (f x)) ls).

Lemma negb_andb : forall p q : bool,
  negb (andb p q) = orb (negb p) (negb q).
Proof.
  destruct p; destruct q; reflexivity.  Qed.

Theorem existsb_existsb' : forall (X : Type) (f : X -> bool) (ls : list X),
  existsb f ls = existsb' f ls.
Proof.
  unfold existsb'. induction ls as [| h ls'].
  Case "ls = []". reflexivity.
  Case "ls = h :: ls'".
    simpl.
    rewrite IHls'.
    rewrite negb_andb. rewrite negb_involutive. reflexivity.  Qed.
(** [] *)

(** $Date: 2014-12-31 16:01:37 -0500 (Wed, 31 Dec 2014) $ *)
(** $Done: 2016-03-12 04:27:29 -0500 (Sat, 12 Mar 2016) $ *)