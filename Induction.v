...
[induction] generates inductive hypothesises, while [destruct] does not.
*)
(** [] *)
...
Proof.
  ...
  Case "n = S n'".
    simpl. rewrite mult_n_Sm. rewrite IHn'. reflexivity.  Qed.
(** [] *)
...
Proof.
  intro n. simpl. rewrite plus_0_r. reflexivity.  Qed.
...
Theorem nat_to_bin_to_nat_ok : forall n : nat,
...
(* The problem is that several binary numbers map to 0, but 0 cannot
   maps to many binary numbers, as nat_to_bin is a function *)

Fixpoint bin_norm (b : bin) : bin :=
  match b with
  | bZ => bZ
  | bO b' => match bin_norm b' with
             | bZ => bZ
             | b'' => bO b''
             end
  | bI b' => bI (bin_norm b')
...
Lemma nat_to_bin_double_ok : forall n,
  n <> O ->
  nat_to_bin (n + n) = bO (nat_to_bin n).
Proof.
  intros n H.
  destruct n as [| n' ].
  Case "n = 0".
    congruence.
  Case "n = S n'".
    clear H. induction n' as [| n'' ].
    SCase "n' = 0".
      reflexivity.
    SCase "n' = S n''".
      rewrite <- !plus_n_Sm in *.
      simpl in *. rewrite IHn''. reflexivity.  Qed.

Lemma incr_non_zero : forall b,
  incr b <> bZ.
Proof.
  intro b. destruct b as [| b' | b' ].
  Case "b = bZ".
    simpl. congruence.
  Case "b = bO b'".
    simpl. congruence.
...
Theorem bin_to_nat_to_bin_norm_ok : forall b : bin,
  nat_to_bin (bin_to_nat b) = bin_norm b.
Proof.
  intro b. induction b as [| b' | b' ].
  Case "b = bZ".
    reflexivity.
  Case "b = bO b'".
    simpl.
    rewrite plus_0_r.
    destruct (bin_to_nat b') eqn: H.
    SCase "bin_to_nat b' = 0".
      rewrite <- IHb'. reflexivity.
    SCase "bin_to_nat b' = S n".
      rewrite nat_to_bin_double_ok.
      rewrite IHb'.
      destruct (bin_norm b') as [| nb' | nb' ] eqn: Hb'.
      SSCase "bin_norm b' = bZ".
        simpl in *. exfalso. eapply incr_non_zero. eapply IHb'.
      SSCase "bin_norm b' = bO nb'".
        reflexivity.
      SSCase "bin_norm b' = bI nb'".
        reflexivity.
      congruence.
  Case "b = bI b'".
    simpl.
    ...
    SCase "bin_to_nat b' = S n".
      rewrite nat_to_bin_double_ok.
      rewrite IHb'. reflexivity. congruence.  Qed.
(** [] *)
...
(** $Done: 2016-03-10 16:07:55 -0500 (Thu, 10 Mar 2016) $ *)
